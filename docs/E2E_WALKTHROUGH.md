-- layout: single title: "E2E WALKTHROUGH" sidebar: nav: "docs" --

# StratoSort Core — End-to-End Walkthrough

A visual guide to every screen and feature in StratoSort Core, captured from the production Linux
AppImage (v2.0.4). Each section maps to one phase of the app workflow.

> **Platform shown:** Ubuntu Linux, AppImage distribution **Launch command:**
...
On first launch, StratoSort checks for AI models in `~/.config/stratosort-core/models/`. If required
models are missing, a setup wizard guides you through downloading them.
...
![Model Setup Wizard](assets/e2e-screenshots/13_model_setup_wizard_detailed.webp)

<!-- Video: assets/e2e-videos/03_discover_analysis_and_model_setup.mp4 -->

The wizard shows installation status for each model (Installed / Not installed) with download size
estimates. Three action buttons are available:
...
During download, you see per-model progress bars with download speed and completion percentage. You
can continue using the app while models download in the background.

![Model Download Progress](assets/e2e-screenshots/10_model_download_wizard.webp)
...
![Welcome Screen](assets/e2e-screenshots/01_welcome_screen.webp)
...
![How StratoSort Works](assets/e2e-screenshots/02_how_stratosort_works_modal.webp)
...
Smart Folders are the destinations the AI uses to organize your files. Each folder has a name, path,
and description that the AI reads to decide where files belong.

![Setup Phase](assets/e2e-screenshots/03_setup_smart_folders.webp)

### Default Folder

A default "Uncategorized" folder is created at `~/StratoSort/Uncategorized` for files that don't
match any other folder.

### Adding a Smart Folder

Click **+ Add Folder** to open the Add Smart Folder dialog:

![Add Smart Folder](assets/e2e-screenshots/04_add_smart_folder_dialog.webp)
...
This phase is where you add files for AI analysis. StratoSort reads file contents, extracts meaning,
and generates organization suggestions.

![Discover Phase](assets/e2e-screenshots/05_discover_phase.webp)
...
Opens a file picker. Navigate to any directory, select individual files or use Ctrl+A to select all,
then confirm.

![File Selection](assets/e2e-screenshots/08_file_selection_dialog.webp)

#### Scan Folder

Opens a folder picker. Select any directory and StratoSort automatically discovers all supported
files within it.

![Scan Folder Picker](assets/e2e-screenshots/31_scan_folder_picker.webp)

### Analysis Progress

Once files are added, AI analysis begins automatically:

![Analysis In Progress](assets/e2e-screenshots/09_analysis_in_progress.webp)
...
![Naming Strategy](assets/e2e-screenshots/06_naming_strategy_modal.webp)
...
![Knowledge OS](assets/e2e-screenshots/07_knowledge_os_panel.webp)
...
The panel requires file embeddings. Click **Build File Embeddings** to generate the vector index
...
After analysis, the Organize phase shows AI-generated suggestions for each file. You review and
approve where each file should be moved and what it should be renamed to.

> **Note:** The Organize tab is only accessible after files have been analyzed in the Discover
> phase. Navigation is enforced — the tab shows a count badge and a tooltip explaining why it may be
> disabled.

### File Grid with AI Suggestions

![Organize Phase](assets/e2e-screenshots/34_organize_phase_ai_suggestions.webp)
...
![Search Widget](assets/e2e-screenshots/16_search_widget.webp)
...
![Knowledge OS Indexed](assets/e2e-screenshots/36_knowledge_os_indexed.webp)

The status bar shows "1 folder · 5 files indexed" confirming embeddings are built. Search tips guide
users on natural language queries.
...
![Invoice Search](assets/e2e-screenshots/37_knowledge_os_search_invoice.webp)
...
![Natural Language Search](assets/e2e-screenshots/38_knowledge_os_search_natural_language.webp)
...
![Understand Tab](assets/e2e-screenshots/46_understand_tab_chat.webp)

The Understand tab is a RAG (Retrieval-Augmented Generation) chat interface for asking questions
about your documents in natural language.
...
![AI Searching](assets/e2e-screenshots/47_understand_ai_searching.webp)
![AI Found Sources](assets/e2e-screenshots/48_understand_ai_found_sources.webp)

#### Relate Tab — Knowledge Graph Visualization

![Relate Tab](assets/e2e-screenshots/50_relate_tab_cluster.webp)

The Relate tab provides an interactive Knowledge Graph for exploring relationships between your
documents. The interface has three panels:
...
![Cluster Details](assets/e2e-screenshots/51_relate_cluster_details.webp)
...
![Cluster Expanded](assets/e2e-screenshots/52_relate_cluster_expanded.webp)

Double-clicking a cluster expands it to show individual file nodes with relationship edges. The
tooltip reads "4 related files. Right-click cluster to organize them."

##### Graph Insights

![Insights](assets/e2e-screenshots/53_relate_insights.webp)
...
![Full Graph with Legend](assets/e2e-screenshots/54_relate_full_graph_legend.webp)

### Building the Search Index

Knowledge OS requires embeddings to function. Embeddings are generated automatically during analysis
when "Embedding Timing" is set to "During analysis" (the default). Two manual rebuild options are
also available:
...
Click the **gear icon** (⚙) in the navigation bar to open Settings. The panel is organized into six
collapsible sections.

![Settings Panel](assets/e2e-screenshots/14_settings_panel_top.webp)

### 8.1 AI Configuration

![AI Configuration](assets/e2e-screenshots/19_settings_ai_config_engine.webp)
...
![Model Management](assets/e2e-screenshots/20_settings_ai_model_management.webp)

Download additional GGUF models by entering the exact filename from the model registry.

#### Embedding Behavior

![Embedding Settings](assets/e2e-screenshots/21_settings_embedding_maintenance.webp)
...
**Rebuild All Embeddings** — Use after changing embedding models. Preserves analysis data while
regenerating the search index.

### 8.2 Performance

![Performance Settings](assets/e2e-screenshots/22_settings_performance_auto_organize.webp)

#### Auto-Organize

- **Auto-organize Downloads** — Automatically analyze and organize new files in your Downloads
  folder
...
- **Max file sizes** — Separate limits for general files (100 MB), images (100 MB), and documents
  (200 MB)

#### Graph Retrieval

![Graph Retrieval](assets/e2e-screenshots/23_settings_graph_retrieval.webp)

- **Enable Graph Expansion** — Use relationship edges to pull related files into search results
- **Graph Expansion Weight** — Slider (0–100%) controlling how much graph neighbors influence
  ranking (default: 20%)
...
![Default Locations](assets/e2e-screenshots/24_settings_default_locations.webp)

- **Default Smart Folder Location** — Base path where new Smart Folders are created (default: home
  directory)
- **Browse** button — Select a different base path

#### File Naming Defaults

![Naming Defaults](assets/e2e-screenshots/25_settings_naming_defaults.webp)

Controls how files are renamed by Download Watcher, Smart Folder Watcher, and Reanalyze All Files.
Same options as the Naming Strategy modal in Discover.

### 8.4 Application

![Application Settings](assets/e2e-screenshots/26_settings_application.webp)
...
![Notification Settings](assets/e2e-screenshots/27_settings_notifications.webp)
...
![Backup & Restore](assets/e2e-screenshots/28_settings_backup_restore.webp)
...
![Analysis History](assets/e2e-screenshots/29_settings_analysis_history.webp)
...
![Diagnostics](assets/e2e-screenshots/30_settings_diagnostics.webp)
...
- **Test All APIs** — Runs connectivity checks against all core services (LLM, vector DB, file
  system, etc.)
...
Five walkthrough videos were recorded from the production AppImage showing real user interactions:
...
On Linux, Background Mode places StratoSort in the system tray. Requires a tray-compatible desktop
environment (GNOME with AppIndicator, KDE, XFCE).
