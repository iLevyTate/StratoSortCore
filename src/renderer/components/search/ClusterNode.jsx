...
const ClusterNode = memo(({ data, selected }) => {
  ...
  const topTerms = sanitizeSemanticTerms(data?.topTerms || data?.commonTags || [], {
    maxTerms: 3,
    minLength: 2
  });
  ...
  return (
    <div
      className={`
        relative rounded-full border-2 shadow-md flex flex-col items-center justify-center overflow-hidden
        transition-all duration-300 cursor-pointer
        ${selected ? 'scale-105 z-50' : 'hover:scale-[1.03] z-10'}
      ...
      style={{
        ...
        height: `${diameter}px`,
        background: selected
          ? 'linear-gradient(135deg, color-mix(in srgb, var(--color-stratosort-accent) 6%, white) 0%, color-mix(in srgb, var(--color-stratosort-accent) 12%, white) 100%)'
          : 'linear-gradient(135deg, color-mix(in srgb, var(--color-stratosort-accent) 3%, white) 0%, color-mix(in srgb, var(--color-stratosort-accent) 8%, white) 100%)',
    ...
    >
      ...
      {/* Center Icon */}
      <div
        className={`p-2 rounded-full mb-1 ${selected ? 'bg-stratosort-accent/20 text-stratosort-accent' : 'bg-stratosort-accent/10 text-stratosort-accent'}`}
      >
      ...
      <div className="text-center px-4 w-full max-w-[82%] mx-auto relative z-10">
        {isEditingLabel ? (
          ...
          <div
            className="text-[11px] font-semibold text-system-gray-800 clamp-2 leading-tight break-words group/label"
            title={label}
          >
            {label}
      ...
      {showTopTerms && (
        <div
          className="mt-0.5 px-4 max-w-[82%] mx-auto text-center text-[10px] leading-tight text-stratosort-accent/90 font-medium clamp-1 break-words"
      ...
      {/* Member Count Badge */}
      <div className="absolute -top-1 -right-1 bg-white border border-stratosort-accent/30 text-stratosort-accent text-[11px] font-semibold px-2 py-0.5 rounded-full shadow-sm flex items-center gap-1">
