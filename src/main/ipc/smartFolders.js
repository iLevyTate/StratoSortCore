...
function registerSmartFoldersIpc(servicesOrParams) {
  ...
  const context = 'SmartFolders';
  const schemaVoid = z ? z.void() : null;
  const schemaString = z ? z.string().min(1) : null;
  const schemaFolder = z ? z.object({}).passthrough() : null;
  const schemaFolderArray = z ? z.array(z.object({}).passthrough()) : null;
  const schemaEditFolder = z ? z.tuple([z.string().min(1), z.object({}).passthrough()]) : null;
  ...
  safeHandle(
    ...
    createHandler({
      logger,
      context,
      schema: schemaEditFolder,
      handler: async (event, folderId, updatedFolder) => {
        try {
  ...
  safeHandle(
    ...
    createHandler({
      ...
      context,
      schema: schemaString,
      handler: async (event, folderName) => {
        try {
